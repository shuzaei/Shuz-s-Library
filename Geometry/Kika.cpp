namespace Kika {
    ...
    bool isParallel(Point a1, Point a2, Point b1, Point b2) { return isParallel(a2 - a1, b2 - b1); }
    ...
    */
    ...
    Polygon convexHull(Polygon s) {
        ...
        inc(i, 2, s.size() - 1) {
            while (u.size() >= 2 and ccw(u[u.size() - 2], u[u.size() - 1], s[i]) != CLOCKWISE) {
                u.pop_back();
            }
            u.push_back(s[i]);
        }
        dec(i, s.size() - 3, 0) {
            while (l.size() >= 2 and ccw(l[l.size() - 2], l[l.size() - 1], s[i]) != CLOCKWISE) {
                l.pop_back();
            }
            l.push_back(s[i]);
        }
